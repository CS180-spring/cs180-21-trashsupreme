...
int main() {
  ...
  cout << endl << "Now outputting all files/folders within \"file_folder\"..." << endl << endl;
  file_folder->outputFolders(file_folder);

  
  cout << endl << "Removing file \"Test.txt\" from folder \"txt\"..." << endl;
  txt_folder1->filemap_remove(txt_file->get_docID());
  cout << "Now searching for removed file in the fileMap..." << endl;
  cout << "Does file exist? (1 = YES, 0 = NO): " << txt_folder1->find_key(txt_file->get_docID()) << endl << endl;
  cout << "Printing files within folder to confirm removal of file..." << endl;
  txt_folder1->outputFolders(txt_folder1);

  cout << endl << "Testing get_folder function... " << endl;
  ...
  cout << endl << "Now searching for folder \"csv_videos\", which is nested within \"csv_data\"..." << endl;
